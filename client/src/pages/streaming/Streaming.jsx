...
const Streaming = () => {
    ...
    const handleVisitClickTS = () => {
        const id = '4';  // Replace with actual ID for Taylor Swift
        navigate(`/profile/${id}`);
    };

    const handleVisitClickPM = () => {
        const id = '6';  // Replace with actual ID for Post Malone
        navigate(`/profile/${id}`);
    };

    const handleVisitClickAG = () => {
        const id = '5';  // Replace with actual ID for Ariana Grande
        navigate(`/profile/${id}`);
    };

    const handleVisitClickNM = () => {
        const id = '7';  // Replace with actual ID for Nicki Minaj
        navigate(`/profile/${id}`);
    };
            ...
            <section>
                <Container>
                    <Row className="d-flex justify-content-around">
                        ...
                        {[
                            { title: "Taylor Swift", img: TS, rating: 4.8, reviews: 2, handler: handleVisitClickTS },
                            { title: "Post Malone", img: P, rating: 4.6, reviews: 1, handler: handleVisitClickPM },
                            { title: "Ariana Grande", img: AG, rating: 4.7, reviews: 3, handler: handleVisitClickAG },
                            { title: "Nicki Minaj", img: NM, rating: 4.5, reviews: 1, handler: handleVisitClickNM }
                        ].map((artist, index) => (
                            <Col lg='3' key={index} className="mb-4">
                                <div className="card">
                                    ...
                                    <div className="card-body">
                                        ...
                                        <button className="btn btn-primary" onClick={artist.handler}>Visit</button>
                                    </div>
                                </div>
                            </Col>
                        ))}
